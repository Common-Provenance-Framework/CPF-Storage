...
@Component
public class MetaDocumentArgumentResolver implements HandlerMethodArgumentResolver {
  private final MetaProvenanceComponentService metaComponentService;

  public MetaDocumentArgumentResolver(
  ...
  private Either<ApplicationException, String> getMetaDocumentIdentifier(MethodParameter parameter, ServerWebExchange exchange) {
    return EITHER.combineM(
        Either.<ApplicationException, LoadMetaDocument> right(parameter.getParameterAnnotation(LoadMetaDocument.class))
            .flatMap(EITHER.makeSureNotNullWithMessage("MethodParameter does not contain LoadMetaDocument annotation.")),
  ...
  public boolean supportsParameter(MethodParameter parameter) {
    return parameter.hasParameterAnnotation(LoadMetaDocument.class)
        && MetaDocument.class.isAssignableFrom(parameter.getParameterType());
  }

  @Override
  public Mono<Object> resolveArgument(MethodParameter parameter, BindingContext bindingContext, ServerWebExchange exchange) {
    return MONO.fromEither(this.getMetaDocumentIdentifier(parameter, exchange))
